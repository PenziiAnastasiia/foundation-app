...
class PDFGeneratorService {
    ...
    static func generateInvoice(amount: Double, fundraiser: String, completion: @escaping (URL?) -> Void) {
        ...
        self.addTitle(in: document, with: invoiceNumber, on: date)
    ...
    static private func addTitle(in document: PDFDocument, with invoiceNumber: Int, on date: String) {
        document.add(.contentCenter, text: "РАХУНОК-ФАКТУРА №\(invoiceNumber)")
        document.add(.contentCenter, text: "від \(date)")
    ...
    static private func addRecipientPayer(in document: PDFDocument, with invoiceNumber: Int, to fundraiser: String, on date: String) {
        ...
        document.add(.contentLeft, text: "Одержувач благодійного внеску:")
        ...
        document.add(.contentLeft, text: "Платник:")
        ...
        document.add(.contentLeft, text: "Директор: \(user.PIB)")
